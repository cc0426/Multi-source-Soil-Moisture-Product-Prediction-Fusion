...
class AttentionAnalyzer:
    ...
    def setup_coordinates(self):
        """Setup latitude and longitude coordinates"""
        # Define latitude and longitude ranges for Northeast China
        self.lat_min, self.lat_max = 38, 54  # 38°N to 54°N
        self.lon_min, self.lon_max = 115, 135  # 115°E to 135°E

        # Get grid dimensions
        self.n_lats, self.n_lons = self.mean_attn.shape[0], self.mean_attn.shape[1]

        # Create coordinate arrays
        # Array index 0 corresponds to lowest latitude (south), index -1 to highest latitude (north)
        self.lats = np.linspace(self.lat_min, self.lat_max, self.n_lats)  # Increasing from south to north
        self.lons = np.linspace(self.lon_min, self.lon_max, self.n_lons)

        print(f"Grid dimensions: {self.n_lats} x {self.n_lons}")
        print(f"Latitude range: {self.lats[0]:.1f}°N to {self.lats[-1]:.1f}°N (array index 0=South, index -1=North)")
        print(f"Longitude range: {self.lons[0]:.1f}°E to {self.lons[-1]:.1f}°E")
    ...
    def plot_spatial_patterns(self):
        ...
        # Self-attention spatial distribution with correct coordinates
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('Self-Attention Weight Spatial Distribution (Northeast China)', fontsize=22)

        for head in range(NUM_HEADS):
            ax = axes[head//2, head%2]
            data = self_attn[..., head]

            # Use imshow with proper extent: [left, right, bottom, top]
            # Data array: row 0 = south (low latitude), row -1 = north (high latitude)
            # So bottom = min latitude (south), top = max latitude (north)
            # Need to flip data vertically because imshow expects row 0 = top
            data_flipped = np.flipud(data)

            im = ax.imshow(data_flipped, cmap='RdYlBu_r',
                          vmin=np.nanpercentile(data, 5),
                          vmax=np.nanpercentile(data, 95),
                          extent=[self.lons[0], self.lons[-1], self.lats[0], self.lats[-1]],
                          aspect='auto',
                          origin='upper')
        ...
        # Error spatial distribution with correct coordinates
        fig, axes = plt.subplots(1, 3, figsize=(18, 5))
        fig.suptitle('Prediction Error Spatial Distribution (Day 1, Northeast China)', fontsize=22)
        ...
        for idx, (ax, error, prod) in enumerate(zip(axes, errors, PRODUCTS)):
            if not np.all(np.isnan(error)):
                ...
                # Flip data vertically because imshow expects row 0 = top
                error_flipped = np.flipud(error)

                im = ax.imshow(error_flipped, cmap='YlOrRd', vmin=vmin, vmax=vmax,
                              extent=[self.lons[0], self.lons[-1], self.lats[0], self.lats[-1]],
                              aspect='auto',
                              origin='upper')
        ...
        # Error difference map with correct coordinates
        if not np.all(np.isnan(self.error_era5_masked)) and not np.all(np.isnan(self.error_colm_masked)):
            ...
            # Flip data vertically because imshow expects row 0 = top
            diff_flipped = np.flipud(diff_era5_colm)

            im = ax.imshow(diff_flipped, cmap='RdBu_r', vmin=-diff_max, vmax=diff_max,
                          extent=[self.lons[0], self.lons[-1], self.lats[0], self.lats[-1]],
                          aspect='auto',
                          origin='upper')
            ...
            ax.set_ylabel('Latitude (°N)')
            ax.grid(True, alpha=0.3, linestyle='--')
            plt.colorbar(im, ax=ax, label='MAE Difference (mm)')
    ...
    def attention_error_correlation(self):
        ...
        for (i, j) in grid_list:
            ...
            grid_stats.append({
                'lat': self.lats[i],  # Use actual latitude value (south to north)
                'lon': self.lons[j],  # Use actual longitude value
                'lat_idx': i,
                'lon_idx': j,
        ...
        if len(grid_stats) > 0:
            ...
            # Scatter plots
            fig, axes = plt.subplots(1, 3, figsize=(18, 5))
            fig.suptitle('Self-Attention Weight vs Prediction Error (Northeast China)', fontsize=22)
    ...
    def analyze_regional_variation(self):
        ...
        fig.suptitle('Attention Pattern Regional Variation (Northeast China)', fontsize=22)

        # Mean self-attention with correct coordinates
        # Flip data vertically because imshow expects row 0 = top
        self_attn_mean_flipped = np.flipud(self_attn_mean)
        im1 = axes[0].imshow(self_attn_mean_flipped, cmap='RdYlBu_r',
        ...
        # Coefficient of variation with correct coordinates
        cv_flipped = np.flipud(cv)
        im2 = axes[1].imshow(cv_flipped, cmap='YlOrRd',
    ...
    def generate_report(self):
        ...
        report.append(f"  - Latitude range: {self.lat_min}°N - {self.lat_max}°N")
        report.append(f"  - Longitude range: {self.lon_min}°E - {self.lon_max}°E")
        report.append(f"  - Grid resolution: {self.n_lats} x {self.n_lons}")
